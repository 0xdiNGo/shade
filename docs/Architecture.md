...
## Cookie verification + mass-op response

Each Shade node feeds the `MODE` and `NOTICE` events it observes into an `OpObserver` (in `shade-bin/op_observer.rs`):

- A `MODE +o nick` from any source seeds a pending op. If a `NOTICE #c :shade-cookie/<wire>` whose payload's `target_nick` matches arrives within `COOKIE_GRACE_MS` (5 s), the op is logged as **certified by cookie** *and the matching `op_history` entry is removed* — so legitimate role-holder ops never feed the mass-op counter. If the grace window passes with no matching cookie, the op is logged as **uncertified** (the signal for a hijacked-bot scenario).
- A sliding `MASS_OP_WINDOW_MS` (10 s) per-`(channel, source)` window flags any source issuing ≥ `MASS_OP_THRESHOLD` (5) **uncertified** ops as a mass-op event. The observer returns a typed `MassOpAction::Deop { channel, source, victims }`, which the daemon turns into one `MODE -o` per victim plus a single `mass_op.deop` audit row. After a trip, further events from the same `(channel, source)` are suppressed for `MASS_OP_COOLDOWN_MS` (60 s) — that ceiling prevents op→deop→op→deop flapping if the rogue source keeps ramming ops.
- The deop response is **gated on `ROLE_OP`** in mesh deployments: only the deterministic role holder for the channel issues the modes. Other peers see the rogue source's ops, observe the role holder's deop in turn, and stay quiet. In single-node mode (no `RoleContext`) the daemon issues the deop directly.

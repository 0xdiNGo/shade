...
| **M5** | Role distribution + cookie ops: `OpRequest`/`OpGrant` mesh messages, HMAC-SHA256 cookies, mass-op/deop detection. | ⏳ **NEXT** | 2 nodes, one with ROLE_OP, the other requests op via mesh, cookie-verified op happens; tampered cookie rejected |

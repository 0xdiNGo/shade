...
- **Probe listener split + mesh self-filter ✅** — `/healthz` and `/readyz` moved from the mTLS admin listener onto the plain-HTTP metrics listener so k8s liveness/readiness probes (and any external uptime poker) can reach them without an operator client cert. Admin listener now serves only `/v1/...`. Mesh `setup_mesh` filters self out of `cfg.mesh.peers` so a single rendered config (k8s ConfigMap) can serve every replica without each pod looping back to itself.
- **k3s + Helm chart ✅** — `deploy/helm/shade/` StatefulSet with headless Service for stable peer DNS, per-replica PVC, rendered `shade.toml` ConfigMap with downward-API node IDs, `shade-pki` and `shade-secrets` Secrets, post-install bootstrap Job (`shade init-ca` + `shade issue-cert` × N + `shade issue-admin-cert`), per-pod cert selection via `select-node-cert` initContainer (so each replica gets its own cert under SAN-binding), liveness/readiness probes on the plain-HTTP metrics listener, admin ClusterIP Service + optional Ingress, optional Prometheus Operator ServiceMonitor, optional NetworkPolicy. `helm lint` clean; Helm chart is the Tier-1 deployment path; Ansible is Tier-2.
- **Backup + restore tooling ✅** — `shade backup` and `shade restore` subcommands in `shade-bin::backup`. Backup uses the rusqlite online backup API (`Backup::run_to_completion`) — WAL-safe, never blocks writers — then verifies the output with `PRAGMA integrity_check`. `--out` accepts a file path, a directory (auto-named `shade-<utc-iso8601>.db`), or absent (binary to stdout). Restore copies atomically (write `.tmp`, fsync, rename), refuses to overwrite without `--force`, refuses if the database is locked by another process (SQLite `BEGIN EXCLUSIVE` probe — no `unsafe` needed), then runs `shade migrate` automatically. 6 new tests in `shade-bin`; operator runbook in `docs/Operations.md` § Backup + restore.

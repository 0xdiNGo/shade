...
**v0.2 backlog** — see `docs/Roadmap.md` § Out of MVP scope: the rest of the in-channel admin flow (`op #foo nick` etc. command handlers; `TOKEN` issuance has shipped), per-handle login rate limit, CRL/OCSP for admin and node certs, the remaining chanset toggles + flood thresholds, multi-network IRC.

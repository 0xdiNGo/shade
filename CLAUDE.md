...
- **k3s + Helm chart ✅** — `deploy/helm/shade/` StatefulSet with headless Service for stable peer DNS, per-replica PVC, rendered `shade.toml` ConfigMap with downward-API node IDs, `shade-pki` and `shade-secrets` Secrets, post-install bootstrap Job (`shade init-ca` + `shade issue-cert` × N + `shade issue-admin-cert`), liveness/readiness probes on the plain-HTTP metrics listener, admin ClusterIP Service + optional Ingress, optional Prometheus Operator ServiceMonitor, optional NetworkPolicy. `helm lint` clean; Helm chart is the Tier-1 deployment path; Ansible is Tier-2.
